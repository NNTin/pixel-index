...
#
# All three steps are idempotent, so running them before every start (not
# just the first) is intended, not wasteful: a restart after a deploy that
# bumped the schema just works, and seeding is a silent no-op once any
# layout exists — seeded or not.
set -eu
